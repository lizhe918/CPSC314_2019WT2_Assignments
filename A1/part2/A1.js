...
function update() {
  ...
  if (timer.value !== 0.0 && timer.value < 100.0) {
    timer.value += 1.0;
  }

  if (timer.value === 100.0) {
    var objGeometry = undefined;
    var obj = undefined;
    var rand = Math.random();
    if (0 <= rand && rand <= 0.33) {
      objGeometry = new THREE.SphereGeometry(1.0, 32.0, 32.0);
      obj = new THREE.Mesh(objGeometry, itemMaterial);
      obj.position.set(magicPosition.value.x, 1.0, magicPosition.value.z);
    } else if (0.33 <= rand && rand <= 0.67) {
      objGeometry = new THREE.TorusGeometry(0.9, 0.4, 10, 20);
      obj = new THREE.Mesh(objGeometry, itemMaterial);
      obj.position.set(magicPosition.value.x, 0.4, magicPosition.value.z);
      obj.rotation.x = Math.PI / 2.0;
    } else {
      objGeometry = new THREE.CylinderGeometry(0.0, 1.0, 2.0, 4.0, 1.0);
      obj = new THREE.Mesh(objGeometry, itemMaterial);
      obj.position.set(magicPosition.value.x, 1.0, magicPosition.value.z);
    }
