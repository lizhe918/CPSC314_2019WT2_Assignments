...
void main() {
	...
	if (timer != 0.0) {
		out_FragColor[0] = out_FragColor[0] + cos(timer);
		out_FragColor[1] = out_FragColor[1] + sin(timer);
		out_FragColor[2] = out_FragColor[2] + cos(cos(timer));
		out_FragColor[3] = out_FragColor[3] + sin(sin(timer));
	}
